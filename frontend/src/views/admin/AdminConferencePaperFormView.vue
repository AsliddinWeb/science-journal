...
// Conference sessions
const conference = ref<{ id: string; title: Record<string, string>; sessions: { id: string; title: Record<string, string> }[] } | null>(null)
...
async function loadPaper() {
  loading.value = true
  try {
    const data = await api.get<any>(`/api/conferences/${confId.value}/papers/${paperId.value}`)
    ...
    if (!data.author_id) {
      ...
      const primary = cos.find((c: any) => c.is_corresponding) ?? cos[0]
      if (primary) {
        guestPrimaryId = primary.id
        form.guest_author_name = primary.guest_name || primary.user?.full_name || ''
        form.guest_author_email = primary.guest_email || primary.user?.email || ''
        form.guest_author_affiliation = primary.guest_affiliation || primary.user?.affiliation || ''
        form.guest_author_orcid = primary.guest_orcid || primary.user?.orcid_id || ''
      }
    }
    ...
    form.co_authors = (data.co_authors || []).filter((a: any) => a.id !== guestPrimaryId).map((a: any) => ({
...
const backUrl = computed(() => `/admin/conferences/${confId.value}/papers`)

onMounted(async () => {
  try { conference.value = await api.get(`/api/conferences/${confId.value}`) } catch {}
...
<template>
  <div class="mx-auto max-w-4xl">
    ...
    <div v-else class="space-y-6">
      ...
      <!-- ── Basic Info ── aynan maqola formidek -->
      <section class="rounded-xl border border-slate-200 bg-white p-6 dark:border-slate-700 dark:bg-slate-800">
        ...
        <!-- Author selector — aynan maqoladek -->
        <div class="mb-4">
          ...
          <div v-else class="rounded-xl border border-slate-200 bg-slate-50 p-4 dark:border-slate-700 dark:bg-slate-800/40">
            ...
            <p class="mt-3 text-[11px] text-slate-400">
              Ushbu muallif tizimda foydalanuvchi sifatida ro'yxatdan o'tmaydi — corresponding muallif sifatida biriktiriladi.
            </p>
          </div>
        </div>
      </section>

      <!-- ── Title & Abstract ── aynan maqoladek -->
      <section class="rounded-xl border border-slate-200 bg-white p-6 dark:border-slate-700 dark:bg-slate-800">
        ...
        <div class="mb-4 flex gap-1 rounded-lg border border-slate-200 bg-slate-50 p-1 dark:border-slate-700 dark:bg-slate-800/50">
          <button v-for="lang in (['uz', 'ru', 'en'] as const)" :key="lang" class="rounded-md px-4 py-1.5 text-sm font-medium transition" :class="langTab === lang ? 'bg-white shadow-sm text-slate-900 dark:bg-slate-700 dark:text-white' : 'text-slate-500 hover:text-slate-700'" @click="langTab = lang">{{ lang.toUpperCase() }}</button>
        </div>
        <div class="mb-4">
        ...
        <div>
          ...
          <textarea v-model="form.abstract[langTab]" rows="6" class="input-base w-full resize-none" :placeholder="`Annotatsiya (${langTab})...`" />
        </div>
      </section>

      <!-- ── Keywords ── aynan maqoladek -->
      <section class="rounded-xl border border-slate-200 bg-white p-6 dark:border-slate-700 dark:bg-slate-800">
        ...
        <div>
          ...
          <textarea v-model="form.keywords[langTab]" rows="3" class="input-base w-full resize-none" :placeholder="`Kalit so'zlar vergul bilan ajratilgan (${langTab})...`" />
        </div>
      </section>

      <!-- ── PDF Upload ── aynan maqoladek -->
      <section class="rounded-xl border border-slate-200 bg-white p-6 dark:border-slate-700 dark:bg-slate-800">
      ...
      <!-- ── Co-authors ── aynan maqoladek -->
      <section class="rounded-xl border border-slate-200 bg-white p-6 dark:border-slate-700 dark:bg-slate-800">
        <div class="mb-4 flex items-center justify-between">
          <h2 class="text-sm font-semibold uppercase tracking-wider text-slate-500">{{ t('author.submit.add_coauthor') }}</h2>
          <button class="flex items-center gap-1.5 rounded-lg bg-primary-50 px-3 py-1.5 text-xs font-medium text-primary-700 hover:bg-primary-100 dark:bg-primary-950/50 dark:text-primary-300" @click="addCoAuthor">
      ...
      <section class="rounded-xl border border-slate-200 bg-white p-6 dark:border-slate-700 dark:bg-slate-800">
        ...
        <textarea v-model="form.references" rows="8" class="input-base w-full resize-y font-mono text-sm" placeholder="[1] Ivanov I.I. Title of article. Journal Name. 2024;1(1):1-10.&#10;[2] Smith J. Another reference..." />
      </section>

      <!-- Save button -->
      <div class="flex justify-end gap-3 pb-8">
        <AppButton variant="secondary" @click="router.push(backUrl)">{{ t('common.cancel') }}</AppButton>
        <AppButton :loading="saving" @click="save">{{ t('common.save') }}</AppButton>
      </div>
    </div>
  </div>
